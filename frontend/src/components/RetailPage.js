...
const RetailPage = ({ language }) => {
  ...
  const handleWhatsAppClick = (product) => {
    ...
    const message = encodeURIComponent(
      `Merhaba! ${productName} ürününüz hakkında perakende satış fiyatı ve stok bilgisi almak istiyorum.`
    );
    ...
    const fetchData = async () => {
      try {
        ...
        if (productsResponse.ok) {
          ...
          // Only show products with retail prices
          const retailProducts = productsData.filter(product => product.retail_price);
    ...
    <div className="header-spacing">
      ...
      <section className="section section-light">
        <div className="container text-center">
          <h1 className="mb-6">{t.retailProducts}</h1>
                ...
                <div key={product.id} className="group">
                  <div className="card hover:scale-105 transition-transform duration-300">
                    ...
                    <div className="mb-4">
                      {product.retail_price && (
                        <div className="text-lg font-semibold text-primary">
                          ${product.retail_price} <span className="text-sm text-gray-600">({t.retail})</span>
                        </div>
                      )}
                    </div>
                    ...
                    >
                      ...
                      <span className="ml-2">{t.getRetailQuote}</span>
                    </button>
                  </div>
                </div>
              ))}
